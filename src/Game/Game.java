...
public class Game implements Serializable {
	...
	private void buildPlayerArray() {
		...
		for (int i = 1; i <= currentGameSettings.getAmountOfPlayer(); i++) {
			...
			Player player = new Player(playerList, currentGameSettings, name,
					playerNumber);
	...
	private void placeAllShips() {
        ...
        for (int playerNumber = 0; playerNumber < playerList.length; ) {
            ...
            if(player.getIsAI() == false){
	            ...
	            for (int s = 0; s < ships.size();) {
	        	 ...
	        	 coordinateInput = AiChooseCoordinate(player, player);
	            	//Teste Eingabe mit RegEx(^ Anfang, 1 Zahl (1-9) und 1 oder keine Zahl(0-9) und 1 Buchstabe (a-z), $ Ende
	                if (coordinateInput.matches("^[1-9]{1}[0-9]{0,1}[a-z]{1}$")) {
	                    error = false;
	                }  //IO.println("Falsche Eingabe, bitte geben sie zuerst die Nummer und dann den Buchstaben des Feldes ein: ");
	            ...
	            shipOrientation = chooseAiOrientation();
	            //Schiff wird gesetzt
				...
				for (int x = 0; x < playfield.getFieldMatrix()[y].length; x++) {
					if (input.equals(playfield.getFieldMatrix()[y][x]
						...
						for (int i = 0; i < ship.getSize(); i++) {
							try {
								// Abfrage, welche prüft ob das Feld auf der
								// das Schiff gesetzt werden soll, deaktiviert
								// ist. Falls ja:
